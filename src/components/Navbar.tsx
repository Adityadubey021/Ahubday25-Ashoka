...
const Navbar: React.FC = () => {
  ...
  return <nav className={cn('fixed w-full z-50 transition-all duration-300 py-2 px-6 md:px-12', scrolled ? 'bg-white/90 shadow-md backdrop-blur-sm' : 'bg-transparent')}>
      <div className="container mx-auto flex justify-between items-center bg-zinc-500">
        ...
        {/* Register Button */}
        <Button onClick={() => scrollToSection('registration')} className="hidden md:block bg-festival-orange hover:bg-orange-600 text-white font-medium">
          Register Now
        </Button>
      ...
      {/* Mobile Menu */}
      {isMenuOpen && <div className="md:hidden bg-white absolute top-full left-0 right-0 shadow-md p-5">
          <div className="flex flex-col space-y-4">
            ...
            <Button onClick={() => scrollToSection('registration')} className="bg-festival-orange hover:bg-orange-600 text-white font-medium">
              Register Now
            </Button>
          </div>
        </div>}
