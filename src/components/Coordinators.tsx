...
const CoordinatorCard: React.FC<CoordinatorProps> = ({
  ...
  return <Card className="overflow-hidden hover:shadow-xl transition-all">
      <CardContent className="p-0">
        <div className="aspect-square bg-gray-200 overflow-hidden">
          <img src={imageSrc || "https://via.placeholder.com/300?text=Coordinator"} alt={name} className="w-full h-full object-cover object-center" />
        </div>
        <div className="p-6">
          <h3 className="text-xl font-bold mb-1 text-festival-purple">{name}</h3>
          <p className="text-gray-600 mb-4 text-left">{role}</p>
          <div className="space-y-2">
            <div className="flex items-center">
              <Phone size={16} className="text-gray-500 mr-2" />
              <a href={`tel:${phone}`} className="text-sm text-gray-700 hover:text-festival-purple">{phone}</a>
            </div>
            {socialLinks && socialLinks.length > 0 && <div className="flex items-center space-x-2 mt-2">
...
const Coordinators: React.FC = () => {
  const studentCoordinators = [{
    ...
    socialLinks: [{
      ...
      url: "https://www.linkedin.com/in/aditya-dubey-49129a248/"
    }, {
      type: "instagram" as const,
      url: "https://www.instagram.com/aditya__dubey21/"
    }]
  }, {
    name: "Diwakar Patel",
    role: "B.Tech (CSE-AIML)",
    ...
    socialLinks: [{
      type: "linkedin" as const,
      url: "https://www.linkedin.com/in/diwakar-patel-151393277/"
    }, {
      type: "instagram" as const,
      url: "https://www.instagram.com/diwakarpatel453t/"
    }]
  }, {
    ...
    socialLinks: [{
      type: "instagram" as const,
      url: "https://www.instagram.com/swami_singh_smartest/"
    }, {
      type: "instagram" as const,
      url: "https://www.instagram.com/swami_singh_smartest/"
    }]
    ...
    socialLinks: [{
      type: "linkedin" as const,
      url: "https://www.linkedin.com/in/malay-vishwakarma-45a892248/"
    }]
  }, {
    name: "Sobrat Dayal",
    role: "B.Tech (CSE)",
    ...
    socialLinks: [{
      type: "linkedin" as const,
      url: "https://www.linkedin.com/in/sobrat-dayal/"
    }, {
      type: "instagram" as const,
      url: "https://www.instagram.com/sobratdayal2022/"
    }]
  }];
  ...
  return <section id="coordinators" className="py-20 bg-white">
      <div className="container mx-auto px-4">
        ...
        <div className="mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {facultyCoordinators.map((coordinator, index) => <CoordinatorCard key={index} name={coordinator.name} role={coordinator.role} phone={coordinator.phone} />)}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {studentCoordinators.map((coordinator, index) => <CoordinatorCard key={index} name={coordinator.name} role={coordinator.role} phone={coordinator.phone} imageSrc={coordinator.imageSrc} socialLinks={coordinator.socialLinks} />)}
          </div>
        </div>
      </div>
    </section>;
